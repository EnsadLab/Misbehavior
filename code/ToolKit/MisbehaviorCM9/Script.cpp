...
void Script::init(DxlEngine* pEng)
{
  frameTime = 0;
  pauseTime = 0;
  currentTask = 0;
...
bool Script::update(unsigned long t)
{ 
...
void Script::execCmd(const char* strcmd,int value)
{
...
void Script::execTokenDbg(int tok,int value)
{
...
bool Script::taskPause(unsigned long dt)
{
...
bool Script::taskJoint(unsigned int dt)
{
  if(duration<1)
  {
...
bool Script::taskWheel(unsigned int dt)
{
  float dlt = timeCoef*(float)dt;
  float t0  = localTime;
  localTime += dlt;
...
void Script::taskSpeedTest(unsigned long dt)
{
...
void Script::startWheelInterpole()
{
   localTime = 0;
   //TODO speedValue ????
}
...
bool Script::interpoleWheel(unsigned long dt)
{
...
//void Script::startBezier(float d,float g,float b0,float b1)
void Script::startBezier()
{
  localTime = 0;
...
bool Script::bezierGoal(unsigned long dt)
{
  float dlt = speedCoef*(float)dt;
  float t0  = localTime;
  localTime += dlt;
